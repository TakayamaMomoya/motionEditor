...
void CParts::Draw(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)m_pModel->dwNumMat; nCntMat++)
	{
		...
		if (m_bCurrent)
		{
			pMat[nCntMat].MatD3D.Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.8f, 1.0f);
			pMat[nCntMat].MatD3D.Ambient = D3DXCOLOR(0.6f, 0.6f, 0.6f, 1.0f);
			pMat[nCntMat].MatD3D.Emissive = D3DXCOLOR(0.2f, 0.2f, 0.8f, 1.0f);
		}
